...
void TRBTree::Save_Tree(std::ofstream *outfile, Node *node) {
    if (node != nil){
        ...
        strcpy(data.m_key, node->m_key);
        data.is_list = false;
        if(node->left == nil && node->right == nil) data.is_list = true;
...
void TRBTree::Load_Tree(std::ifstream *infile){
    ...
    while(true){
        ...
        if (*infile){
            Node *node;
            try {
                char *kkk = (char*)malloc(sizeof(char) * 257);
                strcpy(kkk, data.m_key);
                node = new Node(data.color, nil, nil, nil, kkk, data.m_value);
            } catch(std::bad_alloc&) {
                throw std::bad_exception{};
            }
            ...
            if (pnode == nil){
                ...
                if (strcmp(pnode->m_key, node->m_key) > 0){
                    pnode->left = node;
                } else if (strcmp(pnode->m_key, node->m_key) < 0){
                    pnode->right = node;
                } /*else {
